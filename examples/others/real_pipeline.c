...
main(int argc, char** argv)
{
    ...
    if (my_rank == 0)
    {
        ...
        int rcv;
        MPI_Recv( &rcv, 1, MPI_INT, proc_n-1, tag, MPI_COMM_WORLD, &status );
    ...
    else
    {
        for( int i = 0; i<5; i++ )
        {
            MPI_Recv( &received[i], 1, MPI_INT, my_rank-1, tag, MPI_COMM_WORLD, &status );
            printf("Message %d run trought all the pipline\n", received[i]);
        }
